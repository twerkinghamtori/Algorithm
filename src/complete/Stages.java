...
public class Stages {
	public static int[] solution(int N, int[] stages) {
		...
		int clear[] = new int[N];
		int notclearcnt=0, ncntsum=0;
		for(int i=0; i<failrate.length; i++) {
			for(int j=0; j<stages.length; j++) {
				if(stages[j]==i+1) notclearcnt++;
			}
			notclear[i]=notclearcnt;
			notclearcnt=0;
		}
		for(int i=0; i<clear.length; i++) {
			if(i==0) clear[i]=stages.length;
			for(int j=0; j<i; j++) {
				ncntsum+=notclear[j];
				clear[i] = stages.length-ncntsum;
			}
			ncntsum=0;
			if(clear[i]==0) {
				failrate[i]=0;
			} else {
				failrate[i] = (double)notclear[i]/(double)clear[i];	
			}
		}
		double[] temp = failrate.clone();
		Arrays.sort(temp);
		for(int i=N-1; i>=0; i--) {
			for(int j=0; j<N; j++) {
				if(temp[i]==failrate[j]) {
					failrate[j]=-1;
					answer[N-(i+1)] = j+1;
					break;
				}
